...
class main23 {
  public static void main(String[] args) {
    ...
    int z = 0;
    LehmerRandom lehmerRand = new LehmerRandom().init(10569343);
    while (z < 100) {
      MathGenerator gen = new MathGenerator().generate(2000, lehmerRand);
      /*for (int j = 0; j < gen.size; j++) {
        System.out.print(String.valueOf((char) gen.arr[j]));
      }
      System.out.println();*/
    ...
    /*String str = "4!";
    Token t = null;
    lexer = new Lexer().init(str.codePoints().toArray(), str.length());
    ASTNode node = new Parser().init(lexer).parse();
